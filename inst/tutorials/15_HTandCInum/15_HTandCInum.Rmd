...
The variable `SentenceMonths` is a column within the `SDNYdrug` data frame. Remember that you can access columns in a data frame with the `$` operator or you can utilize the pipe operator (`%>%`) in conjunction with the `summarize()` function.
</div>
