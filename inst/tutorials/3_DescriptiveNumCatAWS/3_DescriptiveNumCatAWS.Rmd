...
tutorial:
  id: "DescriptiveNumCatAWS.Topic3.AppliedStats"
  version: 1.1
output: 
  learnr::tutorial:
    progressive: TRUE
...
rows1 <- c(32738, 42942, 38984, 49039, 53596, 31421, 13771, 15527)
rows2 <- c(27331, 19261, 23162, 8836, 34787, 52548, 29572, 42574)
rows3 <- c(3547, 13371, 26767, 13916, 34426, 42557, 14999)
...
cut.samples <- data.frame(Sample_One = diamonds$cut[rows1], Sample_Two = diamonds$cut[rows2], Sample_Three = diamonds$cut[c(rows3, 25763)])
```
...
```{r print-samples-df, echo = FALSE, eval = TRUE}
samples
```

1. Use the code block below to compute the mean of `Sample_One`
```{r mean-sample-one, exercise = TRUE}

```

<div id="mean-sample-one-hint">
**Hint:** Remember that you can use `R` as a simple calculator, making use of parentheses for grouping.
</div>

```{r mean-sample-one-check}
grade_result(
...
```{r mean-samp-three, exercise = TRUE}

```

```{r mean-samp-three-check}
grade_result(
...
```{r find-medians, exercise = TRUE}

```

```{r mean-vs-median, echo = FALSE}
question(
...
```{r samples-to-estimate-pop, echo = FALSE}
quiz(
question(
...
```{r my-data, exercise = TRUE, exercise.eval = FALSE}
...
```{r echo = FALSE, eval = TRUE, fig.width = 4.5, fig.height = 2.5, message = FALSE, warning = FALSE, results = FALSE}
ggplot(data = diamonds) + geom_histogram(mapping = aes(x = carat)) + geom_vline(xintercept = mean(diamonds$carat), color = "purple", lwd = 1.75)
ggplot(data = diamonds) + geom_boxplot(mapping = aes(y = carat)) + coord_flip()
```
...
In `R` we can easily compute the standard deviation with the function `sd()`, and IQR with the function `quantiles()` for our samples or for the entire dataset! Recall that our diamond samples are stored in a data frame called `samples`. The code block below is preset to compute the standard deviation and IQR for `Sample_One`. Note that in the `quantiles()` function the `0.25` identifies the 25th percentile ($Q1$) and the `0.75` identifies the 75th percentile ($Q3$). Run the code to find the standard deviation, Q1, and Q2 for `Sample_One`. Once you've done that, edit the existing code to compute these metrics for the other two samples.

```{r compute-sd-iqr, exercise = TRUE, exercise.eval = FALSE}
...
**Remark**: Our third sample of diamond carat sizes contained an extreme outlier. The presence of this outlier drastically impacted the computed mean and standard deviation, but didn't have much (if any) effect on the median or $IQR$. Because of this, we say that the median and $IQR$ are *robust* statistics in the presence of outliers.
...
```{r sd-iqr-campaign, exercise = TRUE}

```
  
<div id="sd-iqr-campaign-hint">
**Hint:** Remember that you can define a list of values in `R` using the `c()` operator, separating values by commas.
</div>
...
#1. Build a frequency table for the observations in the sample of your choosing. 
#2. Compare yours to other students in the class. 
#3. Use your frequency table to construct a *relative frequency table* that contains the proportion of times the level of cut occcured in the sample rather than the simple count.
```
...
```{r summarize-cut, exercise = TRUE, exercise.eval = FALSE}
...
```{r echo = FALSE, eval = TRUE, fig.width = 4, fig.height = 2.5, warning = FALSE, message = FALSE, results = FALSE}
...
* Compute the **interquartile range**: `quantile(<data>, c(0.25, 0.75))`
