...
Say that the researcher took a random sample of 100 inmates in State Penitentiaries across the state of New Hampshire and observed the following results. In the sample, 81 inmates were white, 8 were black, 5 were hispanic, and 6 were of another race. The researcher wants to determine whether this sample of inmates refutes the notion that the state prison population in New Hampshire is reflective of the State's breakdown of population by race. We know that a new random sample of 100 inmates would provide results different than these, so how do we determine whether we have a statistically significant result?
...
Compute the number of prisoners out of the 100 who are expected to be *white*.
...
```{r exp-white-check}
grade_result(
  pass_if(~ .result == (100*0.898))
)
```

###

Compute the number of prisoners out of the 100 who are expected to be *black*.
...
```{r exp-black-check}
grade_result(
  pass_if(~ .result == (100*0.018))
)
```

###

Compute the number of prisoners out of the 100 who are expected to be *hispanic*.
...
grade_result(
  pass_if(~ .result == (100*0.040))
)
```

###

Compute the number of prisoners out of the 100 who are expected to be of other races.
...
```{r compute-p-check}
grade_result(
  pass_if(~ (abs(.result - (1 - pchisq(23.04973, df = 4 - 1))) < 0.0001))
)
```
