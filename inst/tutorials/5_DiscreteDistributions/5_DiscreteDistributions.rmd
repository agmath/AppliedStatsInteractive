...
+ Use the definition of binomial experiments to identify scenarios to which the binomial distribution can be applied and then apply the binomial distribution to find probabilities associated with specified outcomes. 
...
<center> $\displaystyle{\mathbb{P}\left[X \leq k\right] = \sum_{i=1}^{k}{\binom{n}{i}\cdot p^i\left(1 - p\right)^{n-i}} \approx \tt{pbinom(k, n, p)}}$</center>
