...
## Topic 10: Intro to Inference Lab
...
In this lab we would like to learn about these home sales by taking smaller samples from the full population. We'll see how well our small samples estimate the population parameters. The data has been loaded for you in a dataset called `ames`.

###

Use the code block below to answer some basic questions about the `ames` dataset.
```{r explore-ames, echo = FALSE, exercise = TRUE}
...
```{r assign, echo = FALSE, exercise = TRUE}
area <- ames$Gr.Liv.Area
...
Use the code block below to find the mean and median for `price` and `area`. Use your results to answer the questions that follow.
```{r eda-sandbox, echo = FALSE, exercise = TRUE}
...
```{r plot-area-price-dists, echo = FALSE, exercise = TRUE, message = FALSE, warning = FALSE, results = FALSE, fig.width= 4, fig.height= 4}
ggplot() + geom_histogram(mapping = aes(x = price)) + ylab("Count") + xlab("Size (Sq ft)") + ggtitle("Histogram of Home Size")
...
```{r samp1, echo = FALSE, exercise = TRUE}
...
```{r plot-dist-sampl, echo = FALSE, exercise = TRUE}
#Take a sample
samp <- sample(area, 50)
...
If we're interested in estimating the average living area in homes in Ames using the sample, our best single guess is the sample mean. Run the code block below and use the result to answer the question that follows.
```{r mean-samp1, echo = FALSE, exercise = TRUE}
samp <- sample(area, 50)
mean(samp)
```
...
```{r second-sample, echo = FALSE, exercise = TRUE}
...
```{r loop, echo = FALSE, exercise = TRUE}
...
```{r hist-breaks, echo = FALSE, exercise = TRUE, exercise.setup = "loop"}
ggplot() + 
...
```{r simple-loop, echo = FALSE, exercise = TRUE}
total <- 0
...
```{r original-loop, echo = TRUE, exercise = TRUE}
sample_means50 <- rep(NA, 5000)
...
```{r small-loop, echo = FALSE, exercise = TRUE}
...
```{r hist-sample-means-50, echo = FALSE, exercise = TRUE, exercise.setup = "loop"}
...
```{r samp10-samp100, echo = FALSE, exercise = TRUE}
#sample_means50 <- rep(NA, 5000)
...
```{r plot-samps, echo = FALSE, exercise = TRUE}
...
We can calculate a 95% confidence interval for a sample mean by adding and subtracting 1.96 standard errors to the point estimate (See Section 4.2.3 of our book if you are unfamiliar with this formula). Essentially the 1.96 comes from the idea that a 95% confidence interval has an error of 5%, which is distributed equally to the two tails of a normal distribution. 

Use the code block below to take a single sample of size 60 of areas from houses in Ames. Call it `samp`.
```{r samp60, echo = FALSE, exercise = TRUE}
...
Read and understand the code below and then run it.
```{r lots-of-ci, echo = FALSE, exercise = TRUE}
...
Now we'll construct the confidence intervals and plot them with the custom function `plot_ci()`.
```{r plot-many-ci, echo = FALSE, exercise = TRUE}
...
Run the code above multiple times. Do your results change? What happens if you change the sample sizes taken? What conclusions can you draw about the relationship between sample size and the confidence intervals?
