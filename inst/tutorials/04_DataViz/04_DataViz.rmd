...
description: "An introduction to the grammar of graphics, plotting strategies, and using ggplot. This notebook loosely follows Chapter 3 of Wikham and Grolemund's R for Data Science text."
---
...
<div id="trans2-fill-class-hint">
Start with the code above but add a `fill` argument to the `mapping` in the `geom_bar()` plot layer. Note that we need to re-engineer the `trans2` variable since we never saved a new copy of the data frame with this variable in it. That is, we temporarily created that variable just for the plot above. 
</div>
...
**A final note:** These interactive workbooks allow me to hide some of the setup requirements from you so that we can focus on the task at hand -- in this workbook, plotting. When you make the switch to using a standalone R instance you'll need to install and load libraries to expand R's functionality. You can do this with the `install.packages()` and `library()` functions. You would need the `ggplot2` or `tidyverse` libraries as well as `skimr` and `ggmosaic` to run the code for this workbook in a local R session.
