...
## Topic 11: Hypothesis Testing and Confidence Intervals for Categorical Data
...
```{r sim-dream-support, echo = FALSE, exercise = TRUE}
...
<i>**Central Limit Theorem:**</i> When observations are independent and the sample size is sufficiently large, the sample proportion $\hat{p}$ will tend to follow a normal distribution with $\mu = p$ (the true population proportion) and standard error $\displaystyle{S_E = \sqrt{\frac{p\left(1-p\right)}{n}}}$.

###

Use the code block below to answer the questions that follow.
```{r clt-prop-sandbox, echo = FALSE, exercise = TRUE}
...
Use what you learned in the video and your knowledge of the Pew Research study to answer the following questions.
...
Here's one more video from Shannon McLintock (also of Openintro.org) introducing the notion of the hypothesis test.
...
```{r ht-questions-sandbox, echo = FALSE, exercise = TRUE}
