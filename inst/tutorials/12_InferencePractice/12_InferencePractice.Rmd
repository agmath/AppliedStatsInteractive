...
In the Topic 11 workbook you learned more about statistical inference (in the form of hypothesis tests and confidence intervals) for categorical variables. In this workbook we'll continue where we left off at the end of Topic 11. You'll start with a video explaining the [*Standard Error Decision Tree*](www/StdErrorDecisionTree.pdf), and then move on to a few practice problems with the opportunity to view a few worked solutions if you need some help! Make sure to try the problems and mark down the questions you have.

## A Tour of the Decision Tree

From this point on you'll make heavy use of the [*Standard Error Decision Tree*](www/StdErrorDecisionTree.pdf) that was referenced earlier. The tree may look intimidating for now, but it contains almost everything we'll need to get through the majority of what remains in our course. Let's start with a tour.

###

<center><iframe width="560" height="315" src="https://www.youtube.com/embed/XzxE_ORMM54" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture; fs" allowfullscreen></iframe></center>

###

## Problems with Walkthroughs

The two problems in this section have attached video walkthroughs -- try answering the question parts before watching the corresponding walkthrough video. If you run into trouble, hit play on the video and see if you can identify the answers from my walkthrough. Be sure to mark down any questions you have. As a reminder, click below for the detailed versions of the general strategies for inference tasks.
...
**Try It 1:** The result of a poll was that 48% of 331 randomly sampled Americans who decide not to go to college do so because they cannot afford it.

  + Calculate a 90% confidence interval for the proportion of Americans who decide not to go to college because they cannot afford it, and interpret the result in context.

<center>**In Case of Emergency Hit Play**</center>
...
question_radio(
  ...
  answer("t-distribution with $df = n_1 + n_2 - 2$"),
...
```
  ...
  + Suppose we wanted the margin of error for the 90% confidence level to be about 1.5%. How large of a sample would you recommend?
...
```{r try-it-2-questions-codeblock, echo = FALSE, exercise = TRUE}

```

<center>INSERT WALKTHROUGH VIDEO</center>
...
grade_result(
  pass_if(~ abs(.result - 0.9434) < 0.005)
)
```
...
Good work through these first two problems -- remember to ask questions about any parts which you are still unsure of. Now move on to try the homework on [MyOpenMath](http://www.myopenmath.com).
