...
As a reminder, we should always label our plots. In creating the plot below, include the following items.

+ `title`: *Association between Attractiveness and Evaluation Score*
+ `x axis label`: *Average Beauty Rating*
+ `y axis label`: *Course Evaluation Score*
...
Use the code block below to add the line to your plot by adding a layer with `geom_abline()` using the appropriate `slope` and `intercept` arguments. Use the same title and axis labels as your previous plots. 
