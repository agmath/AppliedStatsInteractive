...
## Topic 8: Assessing and Exploring the Normality Assumption
...
```{r head-data, echo = FALSE, exercise = TRUE}
...
```{r subset-mf, echo = FALSE, exercise = TRUE}
mdims <- bdims %>% filter(sex == 1)
...
1.  Make a histogram of men's heights and a histogram of women's heights. How would you compare the various aspects of the two distributions? Check back on the data visualization workbook if you need a refresher on plotting.
```{r plot-hgt-hists, echo = FALSE, exercise = TRUE}
...
To see how accurate that description is, we can plot a normal distribution curve on top of a histogram to see how closely the data follow a normal distribution. This normal curve should have the same mean and standard deviation as the data. The code block below is preset to do this for the distribution of male heights. Explanations of the code appear below: 

+ Compute the mean and standard deviation for female heights
...
+ Plot a normal distribution with the same mean and standard deviation as the male heights have.
...
```{r hist-hgt-plus-normal, echo = FALSE, exercise = TRUE, exercise.eval = FALSE, warning = FALSE, message = FALSE}
mhgtmean <- mean(mdims$hgt)
mhgtsd <- sd(mdims$hgt)
...
```{r qq, echo = FALSE, exercise = TRUE, message = FALSE, results = FALSE}
qqnorm(fdims$hgt)
qqline(fdims$hgt)
```
...
A useful way to address this question is to rephrase it as: what do probability plots look like for data that I *know* came from a normal distribution?  We can answer this by simulating data from a normal distribution using `rnorm` (which will draw random values from a normal distribution). Execute the code below to draw a random sample of `r length(fdims$hgt)` random heights from the assumed normal distribution of female heights. A histogram will be plotted for convenience -- feel free to run the code block multiple times to see what happens.
```{r sim-norm, echo = FALSE, exercise = TRUE}
...
```{r sim-norm-qq-plot, echo = FALSE, exercise = TRUE}
...
```{r qqnormsim, echo = FALSE, exercise = TRUE, fig.height = 8}
qqnormsim(fdims$hgt)
```
...
```{r fwght-normal, echo = TRUE, exercise = TRUE, fig.height = 8}
...
```{r prob-height-norm, echo = FALSE, exercise = TRUE}
...
```{r hgt-emp-probability, echo = FALSE, exercise = TRUE}
...
```{r wgt-norm-prob, echo = FALSE, exercise = TRUE}
...
```{r wgt-emp-prob, echo = FALSE, exercise = TRUE}

```

###

Notice that your probability estimated using the normal distribution is only half of what the empirical probability is.
