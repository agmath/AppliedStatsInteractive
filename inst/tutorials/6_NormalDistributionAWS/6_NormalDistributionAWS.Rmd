...
tutorial:
  id: "NormalDistAWS.Topic6.AppliedStats"
  version: 1.0
output: 
  learnr::tutorial:
    progressive: TRUE
...
Throughout this workbook we'll investigate the probability distribution that is most central to statistics: the normal distribution.  If we are confident that our data are nearly normal, that opens the door to many powerful statistical methods. We'll start by estimating probabilities using the normal distribution as a model and then we'll move on to an application using some real estate data and explore the impacts of assuming the normal model in two different scenarios.
...
+ Compute probabilities of events modeled by a normal distribution.
...
+ **Definition:** An observation's $z$-score is the number of standard deviations it falls above or below the mean. If an observation $x$ comes from a nearly normal population with mean $\mu$ and standard deviation $\sigma$ then we compute $z$-score associated with $x$ as follows: 
...
```{r compute-z-scores, echo = FALSE, exercise = TRUE}
...
```{r compute-percentiles, echo = FALSE, exercise = TRUE}
pnorm(1400, 1068, 210)
```
...
We'll make good use of this second method for a while, but don't forget about standardization and $z$-scores. We'll need that later on! For now, let's move on to practicing with fiding probabilities from a normal distribution using R's `pnorm()` function.
...
grade_result(
  pass_if(~ (abs(.result - pnorm(1.73, 0, 1)) < 0.001))
)
```
</div>
<div class = "col-md-5">
...
grade_result(
  pass_if(~ (abs(.result - pnorm(-1.78, 0, 1)) < 0.001))
)
```
</div>
<div class = "col-md-5">
...
grade_result(
  pass_if(~ (abs(.result - (1 - pnorm(-1.99, 0, 1))) < 0.001))
)
```
</div>
<div class = "col-md-5">
...
grade_result(
  pass_if(~ (abs(.result - (1 - pnorm(2.35, 0, 1))) < 0.001))
)
```
</div>
<div class = "col-md-5">
...
grade_result(
  pass_if(~ (abs(.result - (pnorm(1.88, 0, 1) - pnorm(-1.28, 0, 1))) < 0.001))
)
```
</div>
<div class = "col-md-5">
...
grade_result(
  pass_if(~ (abs(.result - (pnorm(-1.17, 0, 1) +(1 - pnorm(1.59, 0, 1)))) < 0.001))
)
```
</div>
<div class = "col-md-5">
...
grade_result(
  pass_if(~ (abs(.result - (pnorm(-1.91, 0, 1) +(1 - pnorm(1.91, 0, 1)))) < 0.001))
)
```
</div>
<div class = "col-md-5">
...
```{r find-percentile, echo = FALSE, exercise = TRUE}
qnorm(0.95, 1068, 210)
```
...
```{r chick-pox-exp, echo = FALSE, exercise = TRUE}

```

```{r chick-pox-exp-check}
grade_result(
  pass_if(~ (abs(.result - (116*0.9)) < 0.1))
)
```

+ Compute the standard deviation in number of adults in samples of size `r chickPoxSampSize` who will have had chickenpox.
```{r chick-pox-sd, echo = FALSE, exercise = TRUE}

```

```{r chick-pox-sd-check}
grade_result(
  pass_if(~ (abs(.result - (sqrt(116*0.9*0.1))) < 0.1))
)
```

```{r surprised-if, echo = FALSE}
question_checkbox(
  paste0("Would you be surprised if your sample contained at most ", floor((116*0.9) - (3.5*sqrt(116*0.9*0.1))), " adults who have had chicken pox? Why? (select all that are appropriate)"),
...
```{r chick-pox-prob-exact1, echo = FALSE, exercise = TRUE}

```

```{r chick-pox-prob-exact1-check}
grade_result(
  pass_if(~ (abs(.result - (dbinom(floor(0.9*116), 116, 0.9))) < 0.001))
)
```
...
```{r chick-pox-prob-exact2, echo = FALSE, exercise = TRUE}

```

```{r chick-pox-prob-exact2-check}
grade_result(
  pass_if(~ (abs(.result - (dbinom(floor(0.1*116), 116, 0.1))) < 0.001))
)
```
...
```{r chick-pox-prob-atmost, echo = FALSE, exercise = TRUE}

```

```{r chick-pox-prob-atmost-check}
grade_result(
  pass_if(~ (abs(.result - (pbinom(floor(0.75*116), 116, 0.9))) < 0.001))
)
```
...
```{r chick-pox-prob-atleast, echo = FALSE, exercise = TRUE}

```

```{r chick-pox-prob-atleast-check}
grade_result(
  pass_if(~ (abs(.result - ( 1 - pbinom((floor(0.95*116) - 1), 116, 0.9))) < 0.001))
)
```

+ Find the probability that more than `r floor(0.7*chickPoxSampSize)` but less than `r floor(0.9*chickPoxSampSize)` adults in your sample of size `r chickPoxSampSize` will have had chicken pox.
```{r chick-pox-prob-between, echo = FALSE, exercise = TRUE}

```

```{r chick-pox-prob-between-check}
grade_result(
  pass_if(~ (abs(.result - ( pbinom((floor(0.9*116) - 1), 116, 0.9) - pbinom(floor(0.7*116), 116, 0.9))) < 0.001))
)
```

**Practice Problem 2:** Sphia took the Graduate Record Examination (GRE) and scored 160 on the Verbal Reasoning section and 157 on the Quantitative Reasoning section. The mean score on the Verbal Reasoning section for all test takers was 151 with a standard deviation of 7, and the mean score for the Quantitative Reasoning section for all test takers was 153 with a standard deviation of 7.67. Suppose we can assume that both score distributions are nearly normal.

+ Use the code block below to compute Sophia's $z$-score on the Quantitative Reasoning exam.
```{r z-score-QR, echo = FALSE, exercise = TRUE}
...
```{r z-score-VR, echo = FALSE, exercise = TRUE}
...
```{r GRE-QR-percentile, echo = FALSE, exercise = TRUE}

```

```{r GRE-QR-percentile-check}
grade_result(
  pass_if(~ (abs(.result - pnorm(157, 153, 7)) < 0.001))
)
```

+ Find the proportion of test takers doing *better than* Sophia on the Verbal Reasoning exam.
```{r GRE-VR-percent-above, echo = FALSE, exercise = TRUE}

```

```{r GRE-VR-percent-above-check}
grade_result(
  pass_if(~ (abs(.result - (1 - pnorm(160, 151, 7.67))) < 0.001))
)
```
