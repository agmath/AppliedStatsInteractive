...
```{r print_samples_df, echo = FALSE, eval = TRUE}
samples
```

1. Use the code block below to compute the mean of `Sample_One`
```{r mean_sample_one, exercise = TRUE}

```

```{r mean_sample_one-check}
grade_result(
...
```{r mean_samp_three, exercise = TRUE}

```

```{r mean_samp_three-solution}
mean(samples$Sample_Three)
```

```{r mean_samp_three-check}
grade_code()
```
...
```{r find_medians, exercise = TRUE}

```

```{r mean_vs_median, echo = FALSE}
question(
...
```{r samples_to_estimate_pop, echo = FALSE}
quiz(
question(
...
```{r my_data, exercise = TRUE, exercise.eval = FALSE}
...
```{r print_plots, echo = FALSE, eval = TRUE, fig.width = 4.5, fig.height = 2.5, message = FALSE, warning = FALSE, results = FALSE}
...
```{r compute_sd_iqr, exercise = TRUE, exercise.eval = FALSE}
...
```{r sd_iqr_campaign, exercise = TRUE}
...
```{r sample_cuts, echo = FALSE, eval = TRUE}
cut.samples
```
...
```{r summarize_cut, exercise = TRUE, exercise.eval = FALSE}
...
```{r print_cut_plots, echo = FALSE, eval = TRUE, fig.width = 4, fig.height = 2.5, warning = FALSE, message = FALSE, results = FALSE}
