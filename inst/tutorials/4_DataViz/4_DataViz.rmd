...
Consider the variables for vehicle class (`class`) and drive type (`drv`). 
...
```{r trans-drv-plot-setup}
mpg$trans2 <- str_sub(mpg$trans, 1, -5) #We create a copy of the transmission column but only include either 'auto' or 'manual'
```
...
mosaicplot(mpg$trans2 ~ mpg$drv, main = "Transmission and Drive Types")
```
