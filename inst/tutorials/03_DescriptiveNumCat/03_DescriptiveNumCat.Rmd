...
samples %>%
  summarize_all(quantile, probs = c(0.25, 0.75))
```
...
<div id="sdIqrCampaign-hint">
**Hint:** You'll need to define the lists of values using the `c()` operator here. Your list of values can be dropped directly into the `sd()` or `IQR()` functions -- the use of the pipe is not necessary.
</div>
